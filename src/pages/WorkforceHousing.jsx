...
export default function WorkforceHousing() {
  ...
  useEffect(() => {
    const fetchHomes = async () => {
      try {
        let res = await fetch(`${API_BASE}/homes?type=Workforce Housing`)
    ...
    <div className="min-h-screen bg-[#F8F7F4]">
      ...
      <section className="bg-[#0B1C33] text-white py-16 px-4">
        <div className="max-w-5xl mx-auto text-center">
          <h1 className="text-3xl sm:text-5xl font-extrabold mb-4">Workforce Housing</h1>
          <p className="text-slate-300 text-lg max-w-2xl mx-auto">
            Practical and durable housing solutions designed for employees, contractors,
      ...
      <section className="max-w-7xl mx-auto px-4 py-14">
        <div className="mb-10">
          <h2 className="text-2xl font-bold text-[#0B1C33] mb-3">Why Choose Workforce Housing?</h2>
          <p className="text-slate-600 max-w-3xl">
            These units are built for durability and quick deployment. Ideal for companies
            that need reliable housing for staff near job sites.
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-16 text-slate-500">
            No Workforce Housing units available at the moment.
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {homes.map((home) => (
              ...
              >
                <div className="h-56 overflow-hidden">
                  <img
                    src={getImageUrl(home.image) || 'https://images.unsplash.com/photo-1600566753086-00f18fb6b3ea?w=800'}
                    alt={home.title}
